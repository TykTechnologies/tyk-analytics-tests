...
class Apis_page extends Page {
  ...
  //OAS API PAGE
  get OAS_API_NAME_INPUT() {return new Input_object('//input[@name="info.title"]');}
  get OAS_API_REST_RADIO() {return new Button_object('//label[text()="REST"]');}
  get OAS_LISTEN_PATH_INPUT() {return new Input_object('//input[@name="x-tyk-config.proxy.listen-path.url"]');}
  get OAS_AUTHENTICATION_DROPDOWN() {return new DropDown_object('//span[@title="Select authentication type"]');}
  get OAS_TARGET_URL_INPUT() {return new Input_object('//input[@name="x-tyk-config.proxy.target-url"]');}
