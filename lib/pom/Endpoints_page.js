...
class Endpoints_page extends Page {
  ...
  removeMiddlewareByName(middleware) {
    let selector = $(`//li[@class="applied-middleware__list-item"]//p[text()="${middleware}"]/following-sibling::button`);
    selector.click();
    let options = $('(//span[text()="OPTIONS"])[2]');
    options.waitForClickable();
    options.click();
    let removeMiddleware = $('(//span[text()="Remove Middleware"])[2]');
    removeMiddleware.waitForClickable();
    removeMiddleware.click();
    let removeMiddlewareConfirmation = $('//button[contains(@class, "tyk-button--danger")]//span[text()="Remove Middleware"]');
    removeMiddlewareConfirmation.click();
  }

  changeMiddlewareStatusByName(middleware) {
    let selector = $(`//li[@class="applied-middleware__list-item"]//p[text()="${middleware}"]/following-sibling::button`);
    selector.click();
    let enableToggle = $('//div[@class="middleware-modal__enable-cmd"]//span[@class="tyk-toggle__item-notch tyk-toggle__item-notch--single"]');
    enableToggle.click();
    this.OAS_UPDATE_MIDDLEWARE_BUTTON.click();
  }

  verifyMiddlewareDisabledByName(middleware) {
    let selector = $(`//li[@class="applied-middleware__list-item"]//p[text()="${middleware}"]/preceding-sibling::p[text()="Disabled"]`);
