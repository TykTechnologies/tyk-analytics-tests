...
export const prepareFederationExampleUpstream = () => {
    let isUpstremRunning = waitForFederationUpstream(1);
    if (isUpstremRunning) {
        return true;
    };
    ...
    bashCmdExecutor('(cd graphql-go-tools/examples/federation ; ./start.sh)');
    return waitForFederationUpstream();
};
...
const waitForFederationUpstream = (retryMaxCount = 10) => {
    let retryNumber = 1;
    
    while(retryNumber <= retryMaxCount){        
        let response;
