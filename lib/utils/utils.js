...
const bashScriptExecutor = require('child_process').exec;
...
export const runFederationExample = () => {
    console.debug('>>> Running Federation example');
    const startFederationExampleScript = exec('(cd graphql-go-tools/examples/federation ; sh start.sh)');
};