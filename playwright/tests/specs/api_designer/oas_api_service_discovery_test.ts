...
xtest('Test Service Discovery settings on OAS API designer page', async ({ createUserAndLogin, main_page }) => {
  ...
  const queryEndpoint = 'http://service-discovery.com/endpoint';
  const targetPath = '/api-target-path';
  const timeout = 34;
  const portDataPath = '/port/path';
  const parentDataPath = 'parent';
  const dataPath = 'some.path';

  
  await test.step('Test default Service Discovery settings and mandatory fields', async () => {
    const firstAPI = true;
    const apiName = 'default-values';
    ...
    const queryEndpointErrorMessage = await this.page.locator('//input[@name="x-tyk-api-gateway.upstream.serviceDiscovery.queryEndpoint"]//following::p[1]');
    const portDataPathErrorMessage = await this.page.locator('//input[@name="x-tyk-api-gateway.upstream.serviceDiscovery.portDataPath"]//following::p[1]');
    const parentDataPathErrorMessage = await this.page.locator('//input[@name="x-tyk-api-gateway.upstream.serviceDiscovery.parentDataPath"]//following::p[1]');
    const dataPathErrorMessage = await this.page.locator('//input[@name="x-tyk-api-gateway.upstream.serviceDiscovery.dataPath"]//following::p[1]');
  ...
  await test.step('User can save API with Custom Preset', async () => {
    const firstAPI = true;
    const apiName = 'custom-sd-preset';
    await openOasDesignerPage(firstAPI);
    await apis_page.OAS_ENABLE_SD_TOGGLE.click();
    await apis_page.SIDE_MENU_UPSTREAM_LINK.click();
  ...
  await test.step('User can save API with Mesosphere Preset', async () => {
    const apiName = "mesosphere-preset";
    await openOasDesignerPage(firstAPI);
    await apis_page.OAS_ENABLE_SD_TOGGLE.click();
    await apis_page.SIDE_MENU_UPSTREAM_LINK.click();
  ...
  await test.step('User can save API with Single ETCD Preset', async () => {
    const apiName = "single-etcd-preset";
    await openOasDesignerPage(firstAPI);
    await apis_page.OAS_ENABLE_SD_TOGGLE.click();
    await apis_page.SIDE_MENU_UPSTREAM_LINK.click();
  ...
  await test.step('User can save API with Nested ETCD Preset', async () => {
    const apiName = "nested-etcd-preset";
    await openOasDesignerPage(firstAPI);
    await apis_page.OAS_ENABLE_SD_TOGGLE.click();
    await apis_page.SIDE_MENU_UPSTREAM_LINK.click();
